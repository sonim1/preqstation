...
function buildAutoPrCompletionError(params: {
  ...
  return [
    ...
    `Push the branch, create a PR targeting ${params.defaultBranch} with gh auth or another authenticated GitHub integration, then retry completion with branchName and a PR URL so the result payload includes the PR URL.`,
  ].join(' ');
}
