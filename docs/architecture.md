...
- Hermes-targeted task, QA, and insight sends use `/preqstation_dispatch`, with the optional
  configured Hermes bot id kept on the command as `/preqstation_dispatch@<botid>`
...
- Valid model overrides are appended as `model` metadata to OpenClaw
  ...
  including the optional `/preqstation_dispatch@<botid>` command suffix form, for task dispatch,
