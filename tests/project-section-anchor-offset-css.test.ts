...
describe('project section anchor offset styles', () => {
  it('uses the measured sticky nav height in the anchor scroll margin', () => {
    expect(panelsCss).toMatch(
      /\.sectionAnchor\s*\{[\s\S]*scroll-margin-top:\s*calc\([\s\S]*var\(--app-shell-header-offset, 0px\)[\s\S]*var\(--mantine-spacing-sm\)[\s\S]*var\(--project-section-nav-height, calc\(8rem - var\(--mantine-spacing-sm\)\)\)[\s\S]*\);/,
    );
